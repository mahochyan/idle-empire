...
加载顺序：`config.js` → `levels.js` → `sprites.js` → `math.js` → `ui.js`
...
**训练队列**：`S.queue[unitKey] = {count, timer, reason}`，排队上限=unitCap×10，每秒生产1个，资源在生产完成时扣除，队列暂停时显示原因（资源不足/驻军满）。
